...
<div class="card">
  <div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h4 class="mb-0">List of Events</h4>
    ...
    <div id="eventModal" class="modal" style="display:none;position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,0.5);justify-content:center;align-items:center;z-index:1000;">
      <div style="background:white;padding:20px;border-radius:8px;width:400px;position:relative;">
        ...
        <form id="eventForm">
          ...
          <input type="hidden" name="addedBy" value="{{ Auth::user()->name ?? 'System' }}">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('openModalBtn').addEventListener('click', () => eventModal.style.display = 'flex');
    closeModalBtn.addEventListener('click', () => eventModal.style.display = 'none');

    // AJAX submit for adding event
    ...
    document.querySelectorAll('.view-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.getElementById('view-date-from').value = btn.dataset.dateFrom;
        document.getElementById('view-date-to').value = btn.dataset.dateTo;
    ...
    viewForm.addEventListener('submit', async e => {
      ...
      try {
        const res = await fetch(`/events/${id}/status`, {
          method: 'POST',
          headers: {
